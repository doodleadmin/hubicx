...
async def link_email(payload: LinkEmailIn, user: User = Depends(current_user), session: AsyncSession = Depends(get_session)) -> User:
    email = normalize_email(payload.email)
    existing = await session.scalar(select(User).where(func.lower(User.email) == email, User.id != user.id))
    if existing:
        raise AppError("email_exists", "Такой email уже привязан к другому аккаунту", 409)
    user.email = email
...
async def link_telegram(payload: LinkTelegramIn, user: User = Depends(current_user), session: AsyncSession = Depends(get_session)) -> dict:
    # Browser account is already authenticated by Bearer token; this endpoint keeps API compatibility.
    return await auth_out(session, user)
