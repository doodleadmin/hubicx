...
export default function BottomNav({ tab, lang, go }: { tab: string; lang: string; go: (tab: string) => void }) {
  ...
  const Tab = ({ id, target }: { id: string; target: string }) => <button className={"tab" + (tab === target ? " on" : "")} onClick={() => go(target)}><NavIcon name={id} active={tab === target} /><span>{label(id)}</span></button>;
  return <div className="tabbar-wrap"><div className="tabbar">
    ...
    <button className="fab" onClick={() => go("create")} aria-label="Create">
