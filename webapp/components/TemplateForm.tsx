...
export default function TemplateForm({ code }: { code: string }) {
  ...
  useEffect(() => { let cancelled = false; setLoadingTimedOut(false); async function load() { try { const auth = await initTelegram(); if (cancelled) return; if (!auth.hasTelegramWebApp || !auth.initData) { if (showDebug) setDebug(getTelegramDebugState()); setError("Откройте WebApp через Telegram-бота"); return; } setAuthReady(true); const [t, u] = await Promise.all([api.template(code), api.me()]); if (!cancelled) { setTemplate(t as Template); setUser(u as User); } } catch (e) { if (!cancelled) setError(e instanceof Error ? e.message : "Не удалось загрузить шаблон"); } } void load(); return () => { cancelled = true; }; }, [code]);
