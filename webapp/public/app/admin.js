(function(){
  ...
  const tabs = [
    ['overview','Обзор'],['users','Пользователи'],['tasks','Генерации'],['pricing','Цены'],['packages','Пакеты'],['transactions','Транзакции'],['files','Файлы'],['partners','Партнёры']
  ];
  ...
  function usersView(){let items=state.data.users?.items||[]; const q=state.filters.users.toLowerCase(); if(q) items=items.filter(u=>String(u.telegram_id).includes(q)||String(u.username||'').toLowerCase().includes(q)||String(u.first_name||'').toLowerCase().includes(q)); return layout(`<div class="ad-toolbar"><div class="ad-fld"><span style="color:var(--faint)"><svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><circle cx="8" cy="8" r="6"/><path d="M12.5 12.5L17 17"/></svg></span><input data-filter="users" placeholder="Поиск: telegram_id / username" value="${esc(state.filters.users)}"/></div></div>${table(['ID','Telegram','Username','Баланс','Admin','Safety','Действия'],items.map(u=>{const sf=u.safety_flags||{}; const safety=sf.count?`${badge(sf.review_required?'Проверить':'Флаг')} <span class="ad-muted">${esc(sf.count)}</span>`:'-'; return `<tr><td>${u.id}</td><td>${u.telegram_id}</td><td>${esc(u.username||u.first_name||'')}</td><td><b>${u.balance_credits}</b></td><td>${badge(u.is_admin)}</td><td>${safety}</td><td><div class="ad-row-act"><button class="ad-mini" data-action="balance" data-user="${u.id}" data-name="${esc(u.username||u.telegram_id)}" title="Баланс">$</button><button class="ad-mini" data-action="ledger" data-user="${u.id}" title="Ledger">L</button></div></td></tr>`;}))}`);}
  ...
  function modal(title, body){
    closeModal();
    document.body.insertAdjacentHTML('beforeend',`<div class="ad-modal-backdrop" id="modal" role="presentation"><div class="ad-modal" role="dialog" aria-modal="true" aria-labelledby="modalTitle"><div class="ad-modal-head"><h2 id="modalTitle">${esc(title)}</h2><button class="ad-btn ad-btn-ghost" type="button" data-action="closeModal" aria-label="Закрыть окно" style="height:32px;padding:0 12px;font-size:12px">Закрыть</button></div><div class="ad-modal-body">${body}</div></div></div>`);
    const backdrop=document.getElementById('modal');
    backdrop?.querySelector('[data-action="closeModal"]')?.addEventListener('click',closeModal);
    backdrop?.addEventListener('click',e=>{if(e.target===backdrop) closeModal();});
    document.addEventListener('keydown',handleModalKeydown);
  }

  async function handleClick(e){const el=e.target.closest('[data-tab],[data-action]'); if(!el) return; const tab=el.dataset.tab, a=el.dataset.action; if(tab) return loadTab(tab); if(a==='refresh') return loadTab(); if(a==='logout') return logout(); if(a==='applyTasks') return loadTab('tasks'); if(a==='closeModal') return closeModal();
