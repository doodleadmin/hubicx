...
export default function CreateVideoPage() { const [locale, setLocale] = useState<Locale>("ru"); useEffect(() => { setLocale(getLocale(undefined, getTelegramLanguageCode())); api.me().then((u) => setLocale(getLocale((u as User).language_code, getTelegramLanguageCode()))).catch(() => undefined); }, []); return <Layout><h1 className="hbx-section-title">{t(locale, "create.video")}</h1><p className="hbx-section-subtitle">{t(locale, "create.videoDesc")}</p><div className="mt-4"><CreateTaskGrid locale={locale} category="video" /></div></Layout>; }
